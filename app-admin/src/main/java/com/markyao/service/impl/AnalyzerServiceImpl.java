...
import com.markyao.model.vo.CommentUserAndCdVo;
...
//            String topWords = bufWordCloud.getTopWords();
            ...
            if (f){
                long ftotals;
                if (bufWordCloud.getTotals()==null || bufWordCloud.getTotals()==0){
                    ftotals= videoInfos.stream().mapToLong(VideoInfo::getTotals).sum();
                }else {
                    //todo 填充总数
                    ftotals=-1;
                }
    ...
    private Map<String, Object> analyzerUsers(String[] aids) {
        ...
        List<CommentUserAndCdVo>result0 =new ArrayList<>();
        Map<String,List<CommentUserAndCdVo>>result1=new HashMap<>();
        ...
        for (Map.Entry<String, Set<String>> entry : mp.entrySet()) {
            ...
            if (list.size()>1) {
                ...
                if (list.size()>1){
                    CommentUserAndCdVo commentUserVo=new CommentUserAndCdVo();
                    ...
                    List<CommentUserAndCdVo> l2 = result1.getOrDefault(list1.size() + "", new ArrayList<>());
